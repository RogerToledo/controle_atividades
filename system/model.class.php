<?php
    ...
    class model {
            ...
            if( $_SERVER['REQUEST_METHOD']=='POST' ):
                ...
                if( isset( $_SESSION['hash'] ) && $_SESSION['hash'] == $hash ):
                    ...
                    if(isset($this->Data)):
                        ...
                        $this->Data['div_id'] = (empty($this->Data['div_id']) ? NULL : $this->Data['div_id']);
            ...
            if( $_SERVER['REQUEST_METHOD']=='POST' ):
                ...
                if( isset( $_SESSION['hash'] ) && $_SESSION['hash'] == $hash ):
                    ...
                    if(isset($this->Data)):
                        ...
                        $this->Data['ativ_hora_inicio'] = (empty($this->Data['ativ_hora_inicio'])? NULL : $this->Data['ativ_hora_inicio']);
                        ...
                        if(!is_null($this->Data['ativ_data_termino'])):
                            if(strtotime($this->Data['ativ_data_inicio']) > strtotime($this->Data['ativ_data_termino'])):
                                ...
                                elseif(empty($this->Data['ativ_data_inicio']) || is_null($this->Data['ativ_hora_inicio']) || empty($this->Data['ativ_hora_termino']) || is_null($this->Data['tpate_id']) || is_null($this->Data['tpati_id']) || is_null($this->Data['usr_id']) ):
                    ...
                    if(isset($this->Data)):
                        ...
                        if(!empty($this->Data['ativ_data_termino'])):
                            if(strtotime($this->Data['ativ_data_inicio']) > strtotime($this->Data['ativ_data_termino'])):
                                ...
                                elseif(empty($this->Data['ativ_data_inicio']) || is_null($this->Data['ativ_hora_inicio']) || empty($this->Data['ativ_hora_termino']) || empty($this->Data['tpate_id']) || empty($this->Data['tpati_id']) || empty($this->Data['usr_id']) ):
                                ...
                                elseif( ((int)$this->Data['ativ_corrente_sla'] > (int)$this->Data['tpati_sla']) && ((int)$this->Data['tpati_sla'] <> '00:00:00') ) :
                                    if(empty($this->Data['ativ_motivo_sla'])):
                                        ...
                                        if($update->getResult()):
                                            ...
                                            if(!is_null($this->Data['ativ_data_termino'])):
                                                $pauseFechadas = new Play;
                                                $pauseFechadas->pausaTermino($this->Data['ativ_id']);
                                            endif;
                                            
                                        else:
                                            $this->Result = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>×</a><strong>Falha na Alteração: </strong>#{$this->Data['ativ_id']} não foi alterada.</div>";
                                        endif; 
                                    endif;  
        ...
        public function calcEsforco($ativ_corrente_sla, $tempIni) {
            if(isset($ativ_corrente_sla) || !is_null($ativ_corrente_sla)):
                $timeSla = explode(":", $ativ_corrente_sla); 
                
                $horaSla = $timeSla[0];
                $minSla  = $timeSla[1];
                $timeSla = ($horaSla * 3600) + ($minSla * 60);
            else:
                $timeSla = 0;
            
            endif;
            
            if(isset($tempIni) || !is_null($tempIni)):
                $timeIni = explode(":", $tempIni);
                      
                $horaIni = $timeIni[0] * 3600;
                $minIni  = $timeIni[1] * 60;
        ...
        /*************************************************************************************
         *
         *  CALCULO DO TEMPO GASTO ENTRE ENTRADA ATÉ INICIO
         * 
         * 
         * @var type 
         *************************************************************************************/
        ...
        private $IdTempIni;
        private $ResultTempIni;
        private $DtEntradaTempIni;
        private $DtInicioTempIni;
        private $HrEntradaTempIni;
        private $HrTerminoTempIni;
        private $TotalDiasTempIni;
        private $DiasTempIni;
        private $TempIni;
        //private $Tarde;


        public function calcTempoInicio($id, $dInicio = null, $hInicio = null) {
            $this->IdTempIni = $id;

            $this->setDataTempIni($this->IdTempIni, $dInicio, $hInicio);
            $this->setDiasUteisTempIni();
            $this->corrigeEntradaTempIni();
            $this->corrigeInicioTempIni();

            $diaEntrada = 0;
            $entrada = 0; 

            // Abre e Fecha no mesmo dia.
            if($this->DiasTempIni == 0 && $this->DtEntradaTempIni == $this->DtInicioTempIni):
                $dia = strtotime($this->HrInicioTempIni) - strtotime($this->HrEntradaTempIni);

                $hora   = floor($dia / 3600);
                $min    = floor(($dia - ($hora * 3600)) / 60);
                $minuto = (strlen($min) == 1 ? "0{$min}" : $min);

                $this->TempIni = "{$hora}:{$minuto}:00";

            else:
                // Calculo parcial - Entrada
                if(strtotime($this->HrEntradaTempIni) < strtotime('13:00:00')):
                    $entrada = strtotime('13:00:00') - strtotime($this->HrEntradaTempIni);
                    $diaEntrada = $entrada + (4 * 3600); 

                elseif(strtotime($this->HrEntradaTempIni) > strtotime('14:00:00')):    
                    $entrada = strtotime('18:00:00') - strtotime($this->HrEntradaTempIni);
                    $diaEntrada = $entrada; 

                endif;

                // Calculo parcial - Inicio
                $inicio = strtotime($this->HrInicioTempIni) - strtotime('09:00:00');

                // Calculo Sla
                $diasCompletos = $this->TotalDiasTempIni;

                $horasCompletas = $diasCompletos * 8 * 3600;

                $horasTotal = $diaEntrada + $horasCompletas + $inicio;
                //echo "horasTotal: {$horasTotal} | diaEntrada: {$diaEntrada} | horasCompletas: {$horasCompletas} | termino: {$termino}";
                $hora   = floor($horasTotal / 3600);
                $min    = floor(($horasTotal - ($hora * 3600)) / 60);
                $minuto = (strlen($min) == 1 ? "0{$min}" : $min);
         ...
         * HrTermino
         */
        private function setDataTempIni($id, $dInicio = null, $hInicio = null) {
            //echo 'Id: '.$this->Id.'<br>';
                        
            if(is_null($dInicio)):
                
                $readTempIni = new Read;
                $readTempIni->FullRead("SELECT ativ_data_entrada AS dtEntrada, ativ_data_inicio AS dtInicio, ativ_hora_entrada AS hrEntrada, ativ_hora_inicio AS hrInicio FROM tb_atividades WHERE ATIV_ID = :id", "id={$this->IdTempIni}");
                //var_dump($readTempIni->getResult());
                if($readTempIni->getResult()):
                    $this->DtEntradaTempIni = $readTempIni->getResult()[0]['dtEntrada'];
                    $this->HrEntradaTempIni = $readTempIni->getResult()[0]['hrEntrada'];

                    if(is_null($readTempIni->getResult()[0]['dtInicio'])):
                            $this->DtInicioTempIni = date('Y-m-d');
                            $this->HrInicioTempIni = date('H:i:s');
                            $this->ResultTempIni = 'aberto';
                    else:
                            $this->DtInicioTempIni = $readTempIni->getResult()[0]['dtInicio'];
                            $this->HrInicioTempIni = $readTempIni->getResult()[0]['hrInicio'];
                            $this->ResultTempIni = 'fechado';
                    endif;
                else:
                    $this->ResultTempIni = false;
                endif;
            else:
                $dInicio = Check::DataBanco($dInicio);
                $readTempIni = new Read;
                $readTempIni->FullRead("SELECT ativ_data_entrada AS dtEntrada, ativ_hora_entrada AS hrEntrada FROM tb_atividades WHERE ATIV_ID = :id", "id={$this->IdTempIni}");
                //var_dump($readTempIni->getResult());
                if($readTempIni->getResult()):
                    $this->DtEntradaTempIni = $readTempIni->getResult()[0]['dtEntrada'];
                    $this->HrEntradaTempIni = $readTempIni->getResult()[0]['hrEntrada'];

                    $this->DtInicioTempIni = $dInicio;
                    $this->HrInicioTempIni = $hInicio;
                    //$this->ResultTempIni = 'fechado';
            ...
            //echo 'DtEntrada: '.$this->DtEntradaTempIni.'<br>';
            //echo 'DtTermino: '.$this->DtTermino.'<br>';
         ...
         * @return INT $totalDias = Quantidade de dias úteis.
         */
        private function setDiasUteisTempIni() {
            $entrada = strtotime($this->DtEntradaTempIni);
            $inicio = strtotime($this->DtInicioTempIni);
            $totalDias = 0;
            $readFeriado = new Read;

            $count = 1;

            while($entrada <= $inicio):
                $proximoDia = date('Y-m-d', $entrada);
                $readFeriado->FullRead("SELECT frd_data AS feriado FROM tb_feriados WHERE frd_ativo = 1 AND frd_data = :data", "data={$proximoDia}");
                $numLinhas = $readFeriado->getRowCount();
            ...
            $this->DiasTempIni = $totalDias;

            //echo "Total dias: {$this->TotalDiasTempIni} | Dias: {$this->Dias} <br>";
         ...
         * @return DATETIME $entradaCorrigida.
         */
        private function corrigeEntradaTempIni() {
            if(strtotime($this->HrEntradaTempIni) < strtotime('09:00:00')):
                $this->HrEntradaTempIni = date('H:i:s', strtotime('09:00:00'));

            elseif(strtotime($this->HrEntradaTempIni) >= strtotime('18:00:00')):
                $this->HrEntradaTempIni = date('H:i:s', strtotime('18:00:00'));

            elseif(strtotime($this->HrEntradaTempIni) > strtotime('13:00:00') && strtotime($this->HrEntradaTempIni) <= strtotime('14:00:00')):     
                $this->HrEntradaTempIni = date('H:i:s', strtotime('14:00:00'));

            else:
                $this->HrEntradaTempIni;
            endif;
            //echo 'HrEntrada: '.$this->HrEntradaTempIni.'<br>';
        }
         ...
         * @return DATETIME $terminoCorrigido.
         */
        private function corrigeInicioTempIni() {
            if(strtotime($this->HrInicioTempIni) < strtotime('09:00:00')):
                $this->HrInicioTempIni = date('H:i:s', strtotime('09:00:00'));
            else:
                $this->HrInicioTempIni;
            endif;
            //echo 'HrTermino: '.$this->HrTermino.'<br>';
        }
